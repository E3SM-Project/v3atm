...
!               Added a new function to replace the default interpolaton algorithm. 
!               Noted as advance_trcdata_linoz to replace the advance_trcdata in 
!               linoz_data_adv. The correspondent input file is also changed to 
!               fit the new scheme.
!-------------------------------------------------------------------
module linoz_data
...
!--------
!--------
!For separate treatment of linoz advance
 subroutine advance_trcdata_linoz( flds, file, state, pbuf2d )
    ...
    use tracer_data,  only : read_next_trcdata,interpolate_trcdata,get_model_time!,interpolate_trcdata_linoz
...
!--------------------------------------BLH-----------------------------------  
    ...
    if(present(pbuf2d)) then
       !call interpolate_trcdata_linoz( state, flds, file, pbuf2d )
       call interpolate_trcdata( state, flds, file, pbuf2d )
    else
       !call interpolate_trcdata_linoz( state, flds, file )
